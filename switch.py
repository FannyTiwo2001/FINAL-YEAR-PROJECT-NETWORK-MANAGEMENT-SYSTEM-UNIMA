...
class Switch:
    ...
    def get_stats(self):
        ...
        interface_result = get_bulk(target = self.router_ip, object_types = interface_object_types, community = CommunityData(self.community), count = 5)

        interface_status_object_types = [ObjectType(ObjectIdentity('IF-MIB', 'ifAdminStatus'))]
        interface_status_result = get_bulk(target = self.router_ip, object_types = interface_status_object_types, community = CommunityData(self.community), count = 5)

        interface_op_status_object_types = [ObjectType(ObjectIdentity('IF-MIB', 'ifOperStatus'))]
        interface_operational_status_result = get_bulk(target = self.router_ip, object_types = interface_op_status_object_types, community = CommunityData(self.community), count = 5)
        ...
        interface_in_octects_result = get_bulk(target = self.router_ip, object_types = interface_in_octects_object_types, community = CommunityData(self.community), count = 5)
        ...
        interface_out_octects_result = get_bulk(target = self.router_ip, object_types = interface_out_octects_object_types, community = CommunityData(self.community), count = 5)
        ...
        interface_speed_result = get_bulk(target = self.router_ip, object_types = interface_speed_object_types, community = CommunityData(self.community), count = 5)
